function [ avg_t, avg_field, n_periods ] = Phase_Average_Varts( dt, ts, field, settings )
%%%
%
...
%%%
    ...
    n_periods = ((settings.end_ts - settings.start_ts) * dt) * settings.freq_Hz;
    ...
    if settings.end_ts - settings.start_ts <= n_bins
        warning('Phase averaging requested time window is smaller than two periods.');
        avg_t = ts(settings.start_ts:settings.end_ts);
        avg_field = field(settings.start_ts:settings.end_ts);
        return
    end
